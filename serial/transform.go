...
func Transform[T1, T2 any](srcCollection []T1, dstCollection []T2, unary UnaryTransform[T1, T2]) {
...
func TransformAppendIf[T1, T2 any](srcCollection []T1, pred Predicate[T1], unary UnaryTransform[T1,
	T2]) []T2 {
	var dstCollection []T2
	for _, item := range srcCollection {
		if pred(item) {
...
func TransformCopyIf[T any](srcCollection []T, dstCollection []T, pred Predicate[T], unary UnaryTransform[T,
	T]) {
	for i, item := range srcCollection {
		if pred(item) {
			dstCollection[i] = unary(item)
